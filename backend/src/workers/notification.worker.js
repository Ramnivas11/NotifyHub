...
const worker = new Worker(
  ...
  async (job) => {
    ...
    const claimed = await notificationService.claimNotification(notificationId);
    if (!claimed) {
      // This means another worker (or process) has already
      // claimed this notification and is processing it.
      // We should exit gracefully.
      logger.info("Notification already claimed by another worker", {
        notificationId,
        jobId: job.id,
      });
      return;
...
worker.on("failed", async (job, error) => {
  ...
  if (error.retryable === false || exhausted) {
    try {
      await notificationService.markFailed(prisma, job.data.notificationId);

      logger.info("Notification marked as FAILED", {
        notificationId: job.data.notificationId,
        reason:
          error.retryable === false ? "PERMANENT_ERROR" : "RETRIES_EXHAUSTED",
      });
    } catch (updateError) {
