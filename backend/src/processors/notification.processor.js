...
const providerRoutingService = require("../services/providerRouting.service");
...
class NotificationProcessor {
  async process(notificationId) {
    ...
    // 3. Resolve provider
    const provider = await providerRoutingService.getProvider(notification);
    ...
    try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await notificationService.markSent(tx, notification.id);
      });
      ...
      await prisma.$transaction(async (tx) => {
        await notificationAttemptService.markFailed(tx, attempt.id, error);

        // Only permanent errors should immediately
        // make the whole notification FAILED.
        if (error.retryable) {
          await notificationService.markPending(tx, notification.id);
        } else {
          await notificationService.markFailed(tx, notification.id);
        }
      });
